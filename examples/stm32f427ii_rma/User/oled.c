...
uint8_t key_adc_def = OLED_KEY_NONE;
uint8_t key_adc_def_last = OLED_KEY_NONE;
uint8_t key_adc_same_cnt = 0;

uint8_t key_adc_out = OLED_KEY_NONE;
uint8_t key_adc_recheck_flag = 0;

extern uint32_t adc1_dma_value_raw[4];
extern uint32_t adc2_dma_value_raw[2];

void oled_adc_button_handler(void)
{
    uint32_t key_adc_value = adc2_dma_value_raw[0];

    if (key_adc_value < 10)
    {
        key_adc_def = OLED_KEY_MIDDLE;
    }
    else if (key_adc_value > 700 && key_adc_value < 1000)
    {
        key_adc_def = OLED_KEY_LEFT;
    }
    else if (key_adc_value > 1500 && key_adc_value < 1900)
    {
        key_adc_def = OLED_KEY_RIGHT;
    }
    else if (key_adc_value > 2200 && key_adc_value < 2600)
    {
        key_adc_def = OLED_KEY_UP;
    }
    else if (key_adc_value > 2800 && key_adc_value < 3400)
    {
        key_adc_def = OLED_KEY_DOWN;
    }
    else
    {
        key_adc_def = OLED_KEY_NONE;
    }

    if (key_adc_def == key_adc_def_last)
    {
        key_adc_same_cnt++;
    }
    else
    ...
    if (key_adc_same_cnt >= 4)
    {
        key_adc_out = key_adc_def;
    }

    key_adc_def_last = key_adc_def;

    // test_time_div = HAL_GetTick() - test_time_last;
    // test_time_last = HAL_GetTick();
}

uint8_t oled_adc_button_get(void)
{
    return key_adc_out;
}
